...
def play_select():
    jsonData = request.get_json()
    plays = []
    print(jsonData['league_id'])
    if int(jsonData['select']) == 1:
        # play = Player_statistics.query.order_by(desc(Player_statistics.pt_score)).all()
        play = db.session.query(Player_statistics.player_id,Player.player_name,Player_statistics.pt_id,Player_statistics.team_name,Player_statistics.pt_score,Player_statistics.pt_assist,Player_statistics.pt_foul,Player_statistics.pt_yellow_card,Player_statistics.pt_red_card).\
            filter(Player_statistics.player_id == Player.player_id,Player_statistics.league_id == jsonData['league_id']).\
    ...
    if int(jsonData['select']) == 2:
        play = db.session.query(Player_statistics.player_id, Player.player_name, Player_statistics.pt_id,
                                ...
                                Player_statistics.pt_red_card). \
            filter(Player_statistics.player_id == Player.player_id,
